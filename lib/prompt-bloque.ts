...
export function construirSistema(nivel: Bloque["nivel"], idioma: IdiomaBloque): string {
  return [
    ...
    "- LA CLASE SE NOMBRA SIN FECHA Y SIN 'ÚLTIMA'. El bloque se lee durante semanas, y para entonces el alumno",
    "  ha tenido otras clases: un 'en tu última clase' o 'la semana pasada' escrito hoy es falso dentro de siete",
    "  días. En el título, la intro, las instrucciones y los contextos di 'en clase', 'lo que viste con tu",
    "  profesor', 'lo que trabajaste'; nunca 'tu última clase', 'your last class', 'la clase de ayer' ni la",
    "  fecha. El día y el nombre del profesor los pone la aplicación al lado del bloque, a partir del dato.",
  ...
  return JSON.stringify(
    {
      ...
      nivel,
      minutos: 10,
      intro: `Una o dos frases ${escribe} que expliquen la idea clave del bloque. Si nombras la clase, 'en clase', nunca 'tu última clase' ni una fecha.`,
      ejercicios: [
        reconocer(1),
        reconocer(2),
        reconocer(3),
